    ...
    <main className="container-page section-pad">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
        <div className="rounded-lg border border-ink/10 bg-white p-4 sm:p-6">
          {items.length ? (
            <div className="grid gap-5">
              {items.map((item) => (
                <div key={item.productId} className="grid gap-4 border-b border-ink/10 pb-5 last:border-b-0 last:pb-0 sm:grid-cols-[110px_1fr_auto]">
        ...
        <aside className="h-fit rounded-lg border border-ink/10 bg-white p-6">
          ...
          <Link href="/checkout" className="mt-6 inline-flex w-full justify-center rounded-full bg-navy px-5 py-3 text-sm font-semibold text-white">
            Finalizar compra
          </Link>
        </aside>
      </div>
